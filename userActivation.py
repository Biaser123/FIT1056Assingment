...
class userActivationFrame(tk.Frame):
    ...
    def __init__(self, master, return_page):
        ...
        activate_canvas = tk.Canvas(master=self, width=128, height=128)
        ...
        user_input_label = tk.Label(master=self, text="User to activate/deactivate:")
        user_input_label.grid(row=2, column=0, sticky=tk.E, padx=10, pady=10)

        # Variable and entry for user to activate/deactivate
        self.user = tk.StringVar()
        self.user_entry = tk.Entry(master=self, textvariable=self.user)
        self.user_entry.grid(row=2, column=1, sticky=tk.W, padx=10, pady=10)

        # Buttons to press to either activate or deactivate
        activate_button = tk.Button(master=self, text="Activate",
                                 command=self.activate_user(self.user))
        activate_button.grid(row=3, columnspan=2, padx=10, pady=10)

        deactivate_button = tk.Button(master=self, text="Deactivate",
                                 command=self.deactivate_user(self.user))
        deactivate_button.grid(row=3, columnspan=2, padx=10, pady=10)
    ...
    def return_menu(self):
        ...
        """
        self.place_forget()
        self.return_menu.place(relx=0.5, rely=0.5, anchor=tk.CENTER)